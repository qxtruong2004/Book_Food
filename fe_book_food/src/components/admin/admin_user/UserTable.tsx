...
const UserTable: React.FC<Props> = ({ data, selectedId, onSelect }) => {
            ...
            <div className="table-responsive" style={{ height: "100%" }}>
                <table className="table table-sm mb-0 align-middle" style={{ height: "100%" }}>
                    <thead className="table-light" style={{
                        ...
                        <tr>
                            <th style={{ width: 70, color: "#2e7d32", fontWeight: "600", padding: "16px 12px" }}>
                                <i className="fas fa-hashtag me-1"></i> ID
                            </th>
                            <th style={{ width: 200, color: "#2e7d32", fontWeight: "600", padding: "16px 12px" }}>
    ...
    >
        ...
        <td style={{ padding: "12px" }}>
            <span className="fw-medium" style={{ color: "#2e7d32" }}>{c.username}</span>
        </td>
        ...
        >
            {c.fullName || <span className="text-muted">Không có tên</span>}  
        </td>
        ...
        <td
            ...
            title={c.email}
        >
            {c.email || <span className="text-muted">Không có email</span>}
        </td>
        
        {/* TD 5: Role - Tách riêng, thêm nội dung */}
        <td style={{ padding: "12px", color: "#2e7d32", fontWeight: "500" }}>
            <span className="badge bg-success-subtle text-success">{c.role || "User"}</span>  
        </td>
        
        {/* TD 6: Status - Di chuyển đúng vị trí cuối */}
        <td style={{ padding: "12px", color: "#616161" }}>
            {c.status || <span className="text-muted">Không có mô tả</span>}
        </td>
    </tr>
))}
                    </tbody>
